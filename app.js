...
function overviewInsightBlock(){
  try{
    ...
    let html='';
    if(ci)html+=`<div class="card pad" style="margin-top:14px;border-color:rgba(124,92,255,.28);background:radial-gradient(circle at 90% 0%,rgba(124,92,255,.10),transparent 40%),linear-gradient(180deg,rgba(23,27,37,.98),rgba(16,19,27,.98))"><span class="eyebrow">LEITURA CRUZADA</span><h3 style="font:700 16px var(--display);margin:7px 0 6px">${esc(ci.head)}</h3><p style="color:var(--muted);font-size:12px;line-height:1.65;margin:0">${esc(ci.body)}</p></div>`;
    const cards=[];
...
const BODY_PARTS=[
  ['pescoco','line',72,88,112],['ombros','line',90,60,140],['peito','line',118,74,126],
  ['cintura','line',166,82,118],['abdomen','line',184,80,120],['quadril','line',205,74,126],
  ['biceps','ring',138,150],['coxa','ring',256,122],['panturrilha','ring',350,124]
];
const BODY_HALF='M100,62 C112,62 119,69 121,79 L150,92 C159,97 161,106 159,152 L153,197 C152,205 149,207 145,205 L139,159 C137,121 132,109 123,101 C121,130 118,148 119,165 C120,174 117,178 119,188 L129,208 C133,214 134,232 133,262 L129,360 L125,414 L108,414 L107,300 L102,216 L100,216 Z';
function bodyHologram(active,opts){
  ...
  return `<svg class="body-holo ${interactive?'interactive':''}" viewBox="0 0 200 440" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Mapa corporal">
   ...
   <g filter="url(#bhglow)"><circle class="body-line" cx="100" cy="40" r="21"/><path class="body-line" d="${BODY_HALF}"/><path class="body-line" d="${BODY_HALF}" transform="matrix(-1,0,0,1,200,0)"/></g>
   ${parts}</svg>`;
}
